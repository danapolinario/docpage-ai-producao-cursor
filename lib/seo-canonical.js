...
export function isDocpageTenantSubdomain(host) {
    const h = stripHostname(host);
    return (/\.docpage\.com\.br$/i.test(h) &&
        h !== 'docpage.com.br' &&
...
function externalCanonicalPriority(lp) {
    ...
    if (custom && !isDocpageTenantSubdomain(custom))
        return custom;
    if (chosen && !isDocpageTenantSubdomain(chosen))
        return chosen;
    return null;
}
export function resolveCanonicalHostname(lp, requestHost) {
    const chosen = stripHostname(lp.chosen_domain);
    const custom = stripHostname(lp.custom_domain);

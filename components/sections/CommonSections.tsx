...
export const WhatsAppButton: React.FC<{ phone?: string; landingPageId?: string }> = ({ phone, landingPageId }) => {
  ...
  return (
    <a href={`https://wa.me/55${cleanPhone}`} target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 z-50 bg-[#25D366] hover:bg-[#20bd5a] text-white p-4 rounded-full shadow-2xl transition-all duration-300 hover:scale-110 group flex items-center gap-2 animate-fade-in" aria-label="Falar no WhatsApp" onClick={handleClick}>
          ...
          <ul className="space-y-4 text-gray-400">
            <li className="flex items-center gap-3">
              ...
              {content.contactEmail || briefing.contactEmail ? (
                <a href={`mailto:${content.contactEmail || briefing.contactEmail}`} className="hover:underline">
                  {content.contactEmail || briefing.contactEmail}
                </a>
              ) : (
                <span>-</span>
            ...
            <li className="flex items-center gap-3">
              ...
              {content.contactPhone || briefing.contactPhone ? (
                <a href={`tel:${(content.contactPhone || briefing.contactPhone || '').replace(/[^+\d]/g, '')}`} className="hover:underline">
                  {content.contactPhone || briefing.contactPhone}
                </a>
              ) : (
                <span>-</span>
